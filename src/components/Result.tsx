...
export const Result = ({ title, results }: ResultProps) => {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-200 dark:text-gray-800">
        {title}
      </h1>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-800 dark:bg-gray-300 dark:text-gray-700 text-gray-50">
              <th className="p-3 text-left border border-gray-700 dark:border-gray-400">
                Type
              </th>
              <th className="p-3 text-left border border-gray-700 dark:border-gray-400">
                Language
          ...
          <tbody>
            {results.map((result, index) => {
              const typeAndDur = results.filter(
                (el) =>
                  el.type === result.type && el.duration === result.duration,
              );
              const bestRes = typeAndDur.reduce((best, current) => {
                if (current.wpm > best.wpm) {
                  return current;
                } else if (current.wpm === best.wpm) {
                  return current.completionTime > best.completionTime
                    ? current
                    : best;
                }
                return best;
              ...
              return (
                <tr
                  key={index}
                  className="text-gray-200 transition-colors dark:text-gray-800 dark:hover:bg-gray-300 hover:bg-gray-800"
                >
                        ...
                        <p className="mt-[3px]">
                          ...
                          {result.type !== "quote" &&
                            ...
                            ![10, 25, 50, 100].includes(result.duration) && (
                              <span className="ml-2 text-yellow-400 dark:text-yellow-500">
                                • Custom
                              </span>
                            )}
                        </p>
